...
// TTAÓÃÓÚË¥±ä¼ÆËã£¬Ã»ÓÐ±ÕºÏ»·Â·£¬»ù±¾²»´æÔÚÍêÈ«ÏàµÈµÄË¥±ä³£Êý£¬¹Ê²ÉÓÃµÝÍÆ¹ØÏµÊ½Çó½âÏµÊý×îÎª¸ßÐ§ //
...
//
//		beff = beff * beff_ji / lamda_ii; // °´ÕÕ±ê×¼µÄ¹«Ê½À´¼ÆËãTTAµÄÏà¹ØÏµÊý
//
//		long double alpha_new = 1;
//		long double alpha_sum = 0;
//		for (int ii = 0; ii < alpha.size(); ++ii)
//		{
//			if (lamda_jj == chain_lamda_list_[ii]) // ÕâÀïµÄlamdaÏµÊýµÄ´¦Àí¿ÉÄÜ´æÔÚÎÊÌâ
//			{
...
//		node_alpha_list_.push_back(alpha); // µÚÒ»¸öºËËØ×÷ÎªÊ×¸ö½Úµã£¬ÆäalphaÏµÊýÐèÒª´æ´¢
//		node_beff_list_.push_back(beff); // µÚÒ»¸öºËËØ×÷ÎªÊ×¸ö½Úµã£¬ÆäbeffÏµÊýÐèÒª´æ´¢
...
// ¶ÔÓÚÈ¼ºÄÎÊÌâ£¬²»¿¼ÂÇ±£ÁôÃ¿²½µÄalphaºÍbeff£¨ÄÚ´æ±¬Õ¨£©£¬¶øÊÇÖ»±£ÁôÃ¿¸öºËËØµÄË¥±äºÍ½ØÃæÐÅÏ¢£¬ÔÙÃ¿²½Ê±¼ÆËãalphaºÍbeff
...
//		{
//			if (next_daughter_index == chain_nuclide_id[ii]) // ÕâÀïµÄlamdaÏµÊýµÄ´¦Àí¿ÉÄÜ´æÔÚÎÊÌâ
//			{
//				visited_number ++;
//			}
//		}
...
{
	//--------------------- ¼ÆËãÊ÷¸ùºËËØµÄalphaÏµÊýºÍbeffÏµÊý ---------------------//
	...
	int chain_point_count = 0; // ¼ÆÊý£¬Èç¹ûÕÒµ½Ò»¸ö×ÓºË£¬¼Ó1

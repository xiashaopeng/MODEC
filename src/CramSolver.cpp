...
extern int CMath::Factorial(int N);
extern Complex CMath::pow_C(const Complex &lhs, int pow);
...
void SolveTrans::PfdCramSolver(SpMat &matrix, vector <double> &N, vector<vector <double> > &F, const double &time) {
    const double alpha0 = 2.1248537104952237488E-16;
    const Complex alpha[8] = {
        Complex(-5.0901521865224915650E-07, -2.4220017652852287970E-05),
        Complex(+2.1151742182466030907E-04, +4.3892969647380673918E-03),
    ...
    for (int j = 0; j < 8; ++j) {
		...
		{
			temp.push_back(CMath::Factorial(jj) * pow(time, jj + 1) * alpha[j] / CMath::pow_C(theta[j], jj + 1));
		}
		for (unsigned int i = 0; i < dim; ++i) {
			N_temp[i] = N[i] * alpha[j];
			for (int jj = 0; jj < F.size(); ++jj)
...
// ---------------  各类高阶CRAM方法及CRAM-Laplace方法求解非齐次燃耗方程 -----------------
...
// 拉普拉斯变换方法求非齐次燃耗方程
// 方法有问题，切勿使用
void SolveTrans::IpfCramSolver32(SpMat &matrix, vector <double> &N, vector <double> &F, const double &time) {
	const double alpha0 = 6.932444346272945E-32;
	const Complex alpha[16] = {
		Complex(+2.531738087291248E+03, -2.175335564251554E+05),
		Complex(+1.124951925994460E+02, -2.293141996821969E+02),
	...
	vector<Complex> F_temp(dim);
	vector <Complex> F_eol;
	F_eol.resize(dim);
	...
	for (int j = 0; j < 16; ++j) {
		Complex temp = time / theta[j];
		for (unsigned int i = 0; i < dim; ++i) {
			N_eol[i] = N_eol[i] + F[i] * temp;
		}
		N_temp = matrix.LUEliminationForIpfCram(theta[j], time, N_eol);
		//F_temp = matrix.LUEliminationForIpfCram(theta[j], time, F_eol);
		for (unsigned int i = 0; i < dim; ++i) {
			N_eol[i] = N[i] + 2.0 * (alpha[j] * N_temp[i]).real_;
			//F_eol[i] += 2.0 * (alpha[j] * F_temp[i]).real_;
		}
	}
	for (unsigned int i = 0; i < dim; ++i) {
		N[i] = (N_eol[i].real_)* alpha0;
